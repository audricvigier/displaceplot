...
plot_popdyn_ssb_obs_sim_for_validation <- function(lst_popdyn1=lst_popdyn, ..., namesimu=list(),
  ...
  MAT  <- read.table(file=file.path(general$main.path.ibm, paste("popsspe_",general$namefolderinput,sep=''),
                                    paste("init_maturity_per_szgroup_biolsce1.dat",sep='')), sep="", dec=".", header=TRUE)
  W    <- read.table(file=file.path(general$main.path.ibm, paste("popsspe_",general$namefolderinput,sep=''),
  ...
  # for the obs ssb
  N     <- read.table(file=file.path(general$main.path.ibm,  paste("popsspe_",general$namefolderinput,sep=''),
...
getAggPoplikeFiles <- function(general=general,
                               explicit_pops=0:26,
                               the_baseline ="calib_multipliers_"
){
